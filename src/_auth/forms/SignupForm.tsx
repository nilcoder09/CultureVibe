...
const SignupForm = () => {
        ...
        async function onSubmit(values: z.infer<typeof SignupValidation>) {
            ...
            if(!newUser) {
                return  toast({
                    title: "Somethign went wrong:( Sign up failed, please try again!",
                  })
            }
            const session = await signInAccount({email: values.email, password:values.password}  );
            if(!session) {
                return toast({ title: "Something went wrong:( Sign in failed, please try again!"});
            }
            ...
            <Form {...form}>
                ...
                <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-5 w-full mt-4">
                    ...
                    render={({ field }) => (
                        ...
                        <FormControl>
                            <Input type="password" className="shad-input" placeholder="Enter Password (at least 8 char required)!" {...field} />
                        </FormControl>
                        <FormMessage />
                        </FormItem>
                    )}
                    ...
                    <p className="text-small-regular text-light-2 text-center mt-2">Log in to your Account! 
